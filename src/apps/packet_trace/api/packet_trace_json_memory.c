...
#define _PT_BUFPOOL_DEBUG
#define _PT_BUFPOOL_DEBUG_LEVEL        _PT_BUFPOOL_DEBUG_ERROR

#define _PT_BUFPOOL_DEBUG_TRACE        (0x1)
#define _PT_BUFPOOL_DEBUG_INFO         (0x01 << 1)
#define _PT_BUFPOOL_DEBUG_ERROR        (0x01 << 2)
#define _PT_BUFPOOL_DEBUG_ALL          (0xFF)

#ifdef _PT_BUFPOOL_DEBUG
#define _PT_BUFPOOL_LOG(level, format,args...)   do { \
            if ((level) & _PT_BUFPOOL_DEBUG_LEVEL) { \
                printf(format, ##args); \
            } \
        }while(0)
#else 
#define _PT_BUFPOOL_LOG(level, format,args...)
#endif

/* The following are proportional to number of collectors */

#define _PT_BUFPOOL_MAX_RESPONE_SLICES      20
#define _PT_BUFPOOL_MAX_REPORT_SLICES       4       
...
{
    uint8_t *buffer;
    PTJSON_MEMORY_SIZE size;
    bool inUse;
    time_t timeTaken;
    int command;
} _PT_BUFPOOL_MEMORY_SLICE_t;
...
static struct _memory_pool_
{
    ...
    uint8_t smallBufferPool[_PT_BUFPOOL_MAX_RESPONE_SLICES][PTJSON_MEMSIZE_RESPONSE];
    uint8_t largeBufferPool[_PT_BUFPOOL_MAX_REPORT_SLICES][PTJSON_MEMSIZE_REPORT];

    /* Buffer Descriptors */

    _PT_BUFPOOL_MEMORY_SLICE_t smallSlices[_PT_BUFPOOL_MAX_RESPONE_SLICES];
    _PT_BUFPOOL_MEMORY_SLICE_t largeSlices[_PT_BUFPOOL_MAX_REPORT_SLICES];
...
#define _PT_BUFPOOL_ASSERT_ERROR(condition, errcode) do { \
    if (!(condition)) { \
        _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_ERROR, \
                    "PT Buffer Pool (%s:%d) Invalid Input Parameter  \n", \
                    __func__, __LINE__); \
        return (errcode); \
    } \
} while(0)

#define _PT_BUFPOOL_ASSERT(condition) _PT_BUFPOOL_ASSERT_ERROR((condition), (BVIEW_STATUS_INVALID_PARAMETER))

/* Utility Macros */

#define _PT_BUFPOOL_SMALL_POOL_START   ((ptr_to_uint_t) & (pBufPool->smallBufferPool[0][0]))
#define _PT_BUFPOOL_SMALL_POOL_END     (_PT_BUFPOOL_SMALL_POOL_START + ( (_PT_BUFPOOL_MAX_RESPONE_SLICES-1) * PTJSON_MEMSIZE_RESPONSE))
#define _PT_BUFPOOL_LARGE_POOL_START   ((ptr_to_uint_t) & (pBufPool->largeBufferPool[0][0]))
#define _PT_BUFPOOL_LARGE_POOL_END     (_PT_BUFPOOL_LARGE_POOL_START + ( (_PT_BUFPOOL_MAX_REPORT_SLICES-1) * PTJSON_MEMSIZE_REPORT))


/* Utility Macros for streamlining the buffer pool access */

#define _PT_BUFPOOL_MUTEX_TAKE(lock) do { \
    int rv; \
    _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_TRACE, "PT BUffer Pool : Acquiring Mutex \n"); \
    rv = pthread_mutex_lock(&(lock)); \
    if (rv != 0) { \
        _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_ERROR, "PT BUffer Pool : Acquiring Mutex Failed : %d \n", rv); \
        return BVIEW_STATUS_RESOURCE_NOT_AVAILABLE; \
    } \
 } while(0)

#define _PT_BUFPOOL_MUTEX_RELEASE(lock) do { \
    int rv; \
    _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_TRACE, "PT BUffer Pool : Releasing Mutex \n"); \
    rv = pthread_mutex_unlock(&(lock)); \
    if (rv != 0) { \
        _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_ERROR, "PT BUffer Pool : Releasing Mutex Failed : %d \n", rv); \
        return BVIEW_STATUS_RESOURCE_NOT_AVAILABLE; \
    } \
 } while(0)
...
BVIEW_STATUS ptjson_memory_init(void)
{
    ...
    _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_TRACE, "PT BUffer Pool : Initializing \n");

    /* clear the memory */
    memset(&ptJsonMemoryPool, 0, sizeof (ptJsonMemoryPool));

    /* Initialize the Buffer Descriptors */

    pBufPool = &ptJsonMemoryPool;

    for (index = 0; index < _PT_BUFPOOL_MAX_RESPONE_SLICES; index++)
    {
        pBufPool->smallSlices[index].buffer = &pBufPool->smallBufferPool[index][0];
        pBufPool->smallSlices[index].size = PTJSON_MEMSIZE_RESPONSE;
        pBufPool->smallSlices[index].inUse = false;
        pBufPool->smallSlices[index].timeTaken = 0;
    }

    for (index = 0; index < _PT_BUFPOOL_MAX_REPORT_SLICES; index++)
    {
        pBufPool->largeSlices[index].buffer = &pBufPool->largeBufferPool[index][0];
        pBufPool->largeSlices[index].size = PTJSON_MEMSIZE_REPORT;
...
{
    int index, max;
    _PT_BUFPOOL_MEMORY_SLICE_t *pSlice;

    _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_TRACE,
                 "PT BUffer Pool : Request for allocation of memory size %d \n",
                 memSize);

    /* Validate Input parameters */

    _PT_BUFPOOL_ASSERT(buffer != NULL);
    _PT_BUFPOOL_ASSERT((memSize == PTJSON_MEMSIZE_RESPONSE) ||
                    (memSize == PTJSON_MEMSIZE_REPORT));

    /* setup our data for lookup */

    if (memSize == PTJSON_MEMSIZE_RESPONSE)
    {
        pSlice = &pBufPool->smallSlices[0];
        max = _PT_BUFPOOL_MAX_RESPONE_SLICES;
    }
    else
    {
        pSlice = &pBufPool->largeSlices[0];
        max = _PT_BUFPOOL_MAX_REPORT_SLICES;
    }

    /* Obtain the lock for lookup */
    _PT_BUFPOOL_MUTEX_TAKE(pBufPool->lock);
    ...
    /* Release the mutex */
    _PT_BUFPOOL_MUTEX_RELEASE(pBufPool->lock);
    ...
    if (index != max)
    {
        ...
        *buffer = pSlice->buffer;
        ...
        _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_TRACE,
                     "PT BUffer Pool : Allocated memory[ %"PRI_PTR_TO_UINT_FMT" - index=%d] size %d at %d \n",
                     (ptr_to_uint_t)(*buffer), index, memSize, (int) pSlice->timeTaken);
        return BVIEW_STATUS_SUCCESS;
    }

    _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_ERROR,
                 "PT BUffer Pool : Failed to allocated memory size %d \n",
                 memSize);
 ...
 *           ptjson_memory_allocate(). BVIEW_STATUS_INVALID_MEMORY is returned otherwise
 *********************************************************************/
BVIEW_STATUS ptjson_memory_free(uint8_t *buffer)
{
    ...
    _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_TRACE, "PT Buffer Pool : Returning %" PRI_PTR_TO_UINT_FMT " to pool \n", pointer);

    /* Validate parameters */
    _PT_BUFPOOL_ASSERT(buffer != NULL);

    /* Check the range. */
    _PT_BUFPOOL_ASSERT( (((pointer >= _PT_BUFPOOL_SMALL_POOL_START) && (pointer <= _PT_BUFPOOL_SMALL_POOL_END)) ||
                      ((pointer >= _PT_BUFPOOL_LARGE_POOL_START) && (pointer <= _PT_BUFPOOL_LARGE_POOL_END))) );


    /* check if this pointer is part of the 'small' pool and return to the pool */
    if ( pointer <= _PT_BUFPOOL_SMALL_POOL_END)
    {
        temp = (pointer - _PT_BUFPOOL_SMALL_POOL_START);

        _PT_BUFPOOL_ASSERT_ERROR ( ((temp % PTJSON_MEMSIZE_RESPONSE) == 0),
                               BVIEW_STATUS_INVALID_MEMORY);

        index = temp / PTJSON_MEMSIZE_RESPONSE;
        ...
        if (pSlice->buffer == buffer)
        {
            ...
            _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_TRACE,
                         "PT Buffer Pool : %" PRI_PTR_TO_UINT_FMT " [index %d] returned to RESPONSE pool \n",
                         pointer, index);
            return BVIEW_STATUS_SUCCESS;
        }

        _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_ERROR,
                     "PT Buffer Pool : %" PRI_PTR_TO_UINT_FMT " doesn't seem to have been allocated from RESPONSE pool \n",
                     pointer);
        return BVIEW_STATUS_INVALID_MEMORY;
    }

    /* Because of the above assert, the buffer is indeed with in large pool range */
    if ( pointer <= _PT_BUFPOOL_LARGE_POOL_END)
    {
        temp = (pointer - _PT_BUFPOOL_LARGE_POOL_START);

        _PT_BUFPOOL_ASSERT_ERROR ( ((temp % PTJSON_MEMSIZE_REPORT) == 0),
                               BVIEW_STATUS_INVALID_MEMORY);

        index = temp / PTJSON_MEMSIZE_REPORT;
        ...
        if (pSlice->buffer == buffer)
        {
            ...
            _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_TRACE,
                         "PT Buffer Pool : %" PRI_PTR_TO_UINT_FMT " [index %d] returned to REPORT pool \n",
                         pointer, index);
            return BVIEW_STATUS_SUCCESS;
        }

        _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_ERROR,
                     "PT Buffer Pool : %" PRI_PTR_TO_UINT_FMT " doesn't seem to have been allocated from REPORT pool \n",
                     pointer);
        return BVIEW_STATUS_INVALID_MEMORY;
    }

    _PT_BUFPOOL_LOG(_PT_BUFPOOL_DEBUG_ERROR,
                 "PT Buffer Pool : %" PRI_PTR_TO_UINT_FMT " doesn't seem to have been allocated any of the pools \n",
                 pointer);
...
void ptjson_memory_dump(void)
{
    ...
    printf (" PT Buffer Pool Statistics : Total Memory %d bytes \n\n", (int)sizeof (ptJsonMemoryPool));
    ...
    printf("\n RESPONSE Pool - Total %3d -- In Use : %3d -- Available %3d \n\n",
           _PT_BUFPOOL_MAX_RESPONE_SLICES, inUseCount, (_PT_BUFPOOL_MAX_RESPONE_SLICES - inUseCount));
    ...
    printf("\n REPORT Pool   - Total %3d -- In Use : %3d -- Available %3d \n\n",
           _PT_BUFPOOL_MAX_REPORT_SLICES, inUseCount, (_PT_BUFPOOL_MAX_REPORT_SLICES - inUseCount));
}
