...
BVIEW_STATUS ptjson_get_pt_profile (void *cookie, char *jsonBuffer, int bufLength)
{
  ...
  {
    /* Parsing and Validating 'packet' from JSON buffer */
  ...
  if(PT_PKT == command.req_method)
  {
    JSON_CHECK_VALUE_AND_CLEANUP (command.collection_interval, 0, 120);
  }
  else if (PT_5_TUPLE == command.req_method)
  {
    JSON_CHECK_VALUE_AND_CLEANUP (command.collection_interval, 1, 30);
  }
  else
  {
    if (root != NULL)
