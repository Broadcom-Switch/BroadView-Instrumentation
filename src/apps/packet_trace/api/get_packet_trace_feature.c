...
BVIEW_STATUS ptjson_get_pt_feature (void *cookie, char *jsonBuffer, int bufLength)
{
    ...
    PTJSON_GET_PT_FEATURE_t command;
    ...
    /* Ensure that 'method' in the JSON equals "get-pt-feature" */
    JSON_COMPARE_STRINGS_AND_CLEANUP ("method", &method[0], "get-packet-trace-feature");
    ...
    status = ptjson_get_pt_feature_impl (cookie, asicId, id, &command);
