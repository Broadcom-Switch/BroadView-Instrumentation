...
BVIEW_STATUS ptjson_cancel_pt_profile (void *cookie, char *jsonBuffer, int bufLength)
{
    ...
    cJSON *json_id, *json_context_id, *root, *params;
    ...
    PTJSON_CANCEL_PT_PROFILE_t command;
    ...
    /* Ensure that 'method' in the JSON equals "cancel-packet-trace-profile" */
    JSON_COMPARE_STRINGS_AND_CLEANUP ("method", &method[0], "cancel-packet-trace-profile");
    ...
    /* Parsing and Validating 'pt-enable' from JSON buffer */
    json_context_id = cJSON_GetObjectItem(params, "id");
    JSON_VALIDATE_JSON_POINTER(json_context_id, "id", BVIEW_STATUS_INVALID_JSON);
    JSON_VALIDATE_JSON_AS_NUMBER(json_context_id, "id");
    /* Copy the value */
    command.id = json_context_id->valueint;
    /* Ensure  that the number 'pt-enable' is within range of [0,1] */
    JSON_CHECK_VALUE_AND_CLEANUP (command.id, 1, 100000);


    /* Send the 'command' along with 'asicId' and 'cookie' to the Application thread. */
    status = ptjson_cancel_pt_profile_impl (cookie, asicId, id, &command);
...
BVIEW_STATUS ptjson_cancel_pt_lag_resolution (void *cookie, char *jsonBuffer, int bufLength)
{
    ...
    strncpy (&method[0], json_method->valuestring, JSON_MAX_NODE_LENGTH - 1);
    /* Ensure that 'method' in the JSON equals "cancel-packet-trace-profile" */
    JSON_COMPARE_STRINGS_AND_CLEANUP ("method", &method[0], "cancel-packet-trace-lag-resolution");
    ...
    /* Send the 'command' along with 'asicId' and 'cookie' to the Application thread. */
    status = ptjson_cancel_pt_lag_resolution_impl (cookie, asicId, id, &command);
...
BVIEW_STATUS ptjson_cancel_pt_ecmp_resolution (void *cookie, char *jsonBuffer, int bufLength)
{
    ...
    strncpy (&method[0], json_method->valuestring, JSON_MAX_NODE_LENGTH - 1);
    /* Ensure that 'method' in the JSON equals "cancel-packet-trace-profile" */
    JSON_COMPARE_STRINGS_AND_CLEANUP ("method", &method[0], "cancel-packet-trace-ecmp-resolution");
    ...
    /* Send the 'command' along with 'asicId' and 'cookie' to the Application thread. */
    status = ptjson_cancel_pt_ecmp_resolution_impl (cookie, asicId, id, &command);
