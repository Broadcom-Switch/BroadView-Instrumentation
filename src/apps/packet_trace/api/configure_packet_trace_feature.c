 ...
 *********************************************************************/
BVIEW_STATUS ptjson_configure_pt_feature (void *cookie, char *jsonBuffer, int bufLength)
{

    /* Local Variables for JSON Parsing */
    cJSON *json_jsonrpc, *json_method, *json_asicId;
    cJSON *json_id, *json_ptEnable;
    cJSON  *root, *params;
    ...
    PTJSON_CONFIGURE_PT_FEATURE_t command;
    ...
    /* Ensure that 'method' in the JSON equals "configure-pt-feature" */
    JSON_COMPARE_STRINGS_AND_CLEANUP ("method", &method[0], "configure-packet-trace-feature");
    ...
    json_ptEnable = cJSON_GetObjectItem(params, "packet-trace-enable");
    JSON_VALIDATE_JSON_POINTER(json_ptEnable, "packet-trace-enable", BVIEW_STATUS_INVALID_JSON);
    JSON_VALIDATE_JSON_AS_NUMBER(json_ptEnable, "packet-trace-enable");
    /* Copy the value */
    command.ptEnable = json_ptEnable->valueint;
    /* Ensure  that the number 'pt-enable' is within range of [0,1] */
    JSON_CHECK_VALUE_AND_CLEANUP (command.ptEnable, 0, 1);


    /* Send the 'command' along with 'asicId' and 'cookie' to the Application thread. */
    status = ptjson_configure_pt_feature_impl (cookie, asicId, id, &command);
