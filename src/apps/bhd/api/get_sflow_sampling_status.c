/*********************************************************************
  ...
  ***********************************************************************
  *
  * @filename get_sflow_sampling_status.c 
  *
...
BVIEW_STATUS bhdjson_get_sflow_sampling_status (void *cookie, char *jsonBuffer, int bufLength)
{
    ...
    cJSON *json_ports_array, *json_ports;
    ...
    BHDJSON_GET_SFLOW_SAMPLING_STATUS_t command;
    ...
    /* Ensure that 'method' in the JSON equals "get-sflow-sampling-status" */
    JSON_COMPARE_STRINGS_AND_CLEANUP ("method", &method[0], "get-sflow-sampling-status");
    ...
    memset(&command.ingress_port_mask, 0, sizeof(BVIEW_PORT_MASK_t));
    ...
    for (iter = 0; iter < cJSON_GetArraySize(json_ports_array); iter++)
    {
      ...
      BVIEW_MASKOREQ (command.ingress_port_mask, temp_pbmp, sizeof(BVIEW_PORT_MASK_t));
    }

    /* Send the 'command' along with 'asicId' and 'cookie' to the Application thread. */
    status = bhdjson_get_sflow_sampling_status_impl (cookie, asicId, id, &command);
